    ...
    it("should get chunked users that suit the given condition", function (done) {
        ...
        co(function* () {
            ...
            yield query.whereIn("id", ids).orderBy("id").chunk(5, function (_data) {
                ...
                if (offset) {
                    sql = 'select * from (select *, row_number() over(order by "id") "_rn" from "users" where "id" in (' + Array(20).fill("?").join(", ") + ')) tmp where tmp."_rn" > ' + offset + ' and tmp."_rn" <= ' + (offset + 5);
                } else {
                    sql = 'select * from "users" where "id" in (' + Array(20).fill("?").join(", ") + ') order by "id" fetch first 5 rows only';
                }
