...
describe("Model.prototype.has() & Model.prototype.belongsTo()", function () {
    it("should create model associations as expected", function (done) {
        ...
        co(function* () {
            ...
            assert.equal(article.sql, 'select * from "articles2" where "user_id" = ? fetch first 1 rows only');
            ...
            assert.equal(_user.sql, 'select * from "users2" where "id" = ? fetch first 1 rows only');
            ...
            assert.equal(userTag.sql, 'select * from "tags2" where "taggable_id" = ? and "type" = ? fetch first 1 rows only');
            ...
            assert.equal(user2.sql, 'select * from "users2" where "id" = ? fetch first 1 rows only');
